...
//              newgraphic.FillPath(myBrush, path);
        ...
        private void bk_Generate_Click(object sender, EventArgs e)
        {
            ...
            firstPartition mfirstPartition = new firstPartition();    //创建对象mfirstPartition
            ...
            foreach (List<Vector2> pointsOfIndivualConvexPolygon in listOfConvexPolygonPartionBaya)
            {
                ...
                Path pathConver = NewMethod(currentPolygonVertices);
                subj.Add(pathConver);
            }
        ...
        private Path NewMethod(List<Vector2> currentPolygonVertices)
        {

            //数据先扩大了Scale倍，保存精度，变成IntPoint
            return ConvexPolygonToPath(currentPolygonVertices, 1 / scale);
        }
        ...
        private void bk_Infill_Click(object sender, EventArgs e)
        {
            ...
            Console.WriteLine("调用bk_infill_click函数  凸多边形分区");
            foreach (List<Vector2> pointsOfIndivualConvexPolygon in listOfConvexPolygonPartionBaya)
            {
        ...
        {
            Console.WriteLine("Mesh_bk_click正方形网格填充");
            imgInfoSaves.Clear();
            List<List<Vector2>> newpointLists = new List<List<Vector2>>(inputPologonVectors);
            AABBoriginal = AABBf1(inputPologonVectors[0]);
        ...
        private void TextBox_lineSpace_TextChanged(object sender, EventArgs e)
        {
